...
候选窗口会根据引擎当前的 segment/token 结果显示实际拼音切分，例如 `fang'an`；输入撇号强制分隔时会显示 `xi'an`。

候选排序会使用光标前的已上屏文本作为语言模型上下文。实现沿用 Gboard 的
原生 TARGET_TOKEN 注入流程：中文保留最近 5 个 UTF-16 单元，英文保留最近
20 个，遇到标点、空白或语言边界时截断。例如，无上下文输入 `bushu` 时首选
“部署”，在“我对这里很”之后输入时首选会调整为“不熟”。
